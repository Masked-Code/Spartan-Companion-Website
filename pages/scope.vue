<template>
  <UCard class="rounded-2xl ml-[10%] mr-[10%] mt-[10%] min-h-96">
    <div class="flex flex-col place-items-center">
      <div class="text-4xl font-bold m-1 mb-5">Scope of Operation</div>
      <div>With this tool I seek to build a long lasting solution for getting sailors up to speed and ready to lead.</div>
      <div>A sailor should be able to find the official ultilities they need, and be shown the optimal usage for them.</div>
      <div>A sailor should be able to store a backup of their records via their own volition.</div>
      <div>A sailor should be able to edit and maintain packages with effective supporting information and utilities.</div>
      <div>A sailor should be able to connect and communicate with sailors with similar experiences and needs.</div>
    </div>
  </UCard>
</template>

<script lang="ts" setup>
</script>

<style>

</style>